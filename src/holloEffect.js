...
/**
 * Dot screen shader in postprocessing
 * 
 */
...
// Mix final noise value
  ...
  // tweak 0.5 to control intensity
  float holoIntensity = uHoloIntensity + 0.1 * snoise(vec3(0., 30., uTime));

  // change y to change patterin of hollogran effect
  float direction = mod( vUv.y * uSize.y + uTime * 0.5 , 4.0 ) < 2.0 ? 1.0 : -1.0;
  
  // change 8 to create distanced holo effect
  ...
  vec3 luminance = vec3(0.29, 0.587, 0.114);
  ...
  gl_FragColor = color;
  gl_FragColor = vec4(vec3(theFinalFinalColor), 1.0);
